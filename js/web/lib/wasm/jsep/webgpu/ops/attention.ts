...
const createAttentionProbsProgramInfo = (
  ...
  _totalSequenceLengthInput: TensorView | undefined,
) => {
  ...
  const getShaderSource = (shaderHelper: ShaderHelper) => {
    ...
    const uniforms: UniformsArrayType = [
      ...
      { name: 'present_key_size', type: 'u32' },
    ];
    return `
  ...
  ${shaderHelper.mainStart([TILE_SIZE, TILE_SIZE, 1])}
    ...
    let sequence_length = u32(${seqLens ? seqLensInputVariable?.getByOffset('batchIdx') : 'uniforms.M'});
    ...
    let pastKeyOffset = uniforms.past_sequence_length * uniforms.K * kvHeadIdx;`;
      } else {
        return `
    ...
    for (var w: u32 = 0u; w < uniforms.K; w += TILE_SIZE) {
      ...
      ${
        presentKey
          ...
          if (present_key_idx < uniforms.present_key_size) {
...
const createVxAttentionScoreProgramInfo = (
  ...
  _totalSequenceLengthInput: TensorView | undefined = undefined,
) => {
  ...
  const getShaderSource = (shaderHelper: ShaderHelper) => {
    ...
    const uniforms: UniformsArrayType = [
      ...
      { name: 'kv_sequence_length', type: 'u32' },
      { name: 'n_reps', type: 'u32' },
      { name: 'present_value_size', type: 'u32' },
    ];
    return `
  const TILE_SIZE = ${TILE_SIZE}u;
  ...
  ${shaderHelper.mainStart([TILE_SIZE, TILE_SIZE, 1])}
   ...
   let n = global_id.x;
   let sequence_length = u32(${seqLens ? seqLensInputVariable?.getByOffset('batchIdx') : 'uniforms.M'});
   let offsetA = headIdx * (sequence_length * uniforms.K) + m * uniforms.K;
   ${(() => {
